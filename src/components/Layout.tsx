  ...
  const sidebarContent = (
    <>
      <div className="h-16 px-5 flex items-center border-b border-slate-100 dark:border-white/5">
        ...
        <button
          className="ml-auto md:hidden text-slate-500 hover:text-slate-900 p-2 rounded-xl"
      ...
      <div className="p-4 border-t border-slate-100 dark:border-white/5">
        <div className="relative">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-xl bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:bg-slate-100 dark:hover:bg-white/10 transition-a[...]
          >
            {profile?.avatar_url ? (
              <img
              ...
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center text-white font-bold text-sm ring-2 ring-white dark:ring-slate[...]
          ...
          {isUserMenuOpen && (
            <>
              ...
              />
              <div className="absolute bottom-full left-0 right-0 mb-2 z-50 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in s[...]
                <div className="p-1">
                  <Link
                    to="/profile"
                    onClick={() => setIsUserMenuOpen(false)}
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-dark-bg bg-dots">
      ...
      {isMobileMenuOpen && (
        <>
          ...
          />
          <aside className="fixed top-0 left-0 z-[100] w-72 max-w-[90vw] h-full bg-white dark:bg-slate-900 shadow-2xl glass flex flex-col border-r border-slate-200 dark:border-white/5 animate-in duration-100 transition-all">
            {sidebarContent}
          </aside>
        </>
      )}
      ...
      <div className="flex-1 flex min-w-0 overflow-hidden relative">
        {/* Mobile Header */}
        <header className="md:hidden h-16 flex items-center justify-between px-4 border-b border-slate-200 dark:border-white/5 shrink-0 bg-slate-50 dark:bg-dark-bg sticky top-0 z-30">
          <button
            onClick={() => setIsMobileMenuOpen(true)}
            className="p-2 rounded-full text-slate-700 dark:text-white ring-2 ring-primary-500"
            aria-label="Abrir menu"
          >
            <Menu size={28} />
          </button>
          <span className="text-lg font-bold font-display tracking-tight text-slate-900 dark:text-white">NossoCRM</span>
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative transition-all duration-300 ease-in-out">
          ...
          {/* Desktop Header (mantida) */}
          <header className="hidden md:flex h-16 glass border-b border-slate-200 dark:border-white/5 items-center justify-end px-6 z-10 shrink-0">
        ...
        <div
          className={`border-l border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 transition-all duration-300 ease-in-out overflow-hidden flex flex-col ${isGlobalAIOpen ? 'w-96 opacity-1[...]
        >
